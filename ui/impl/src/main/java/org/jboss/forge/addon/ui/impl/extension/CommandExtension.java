...
import org.jboss.forge.addon.ui.impl.InputComponentProducer;
      ...
      for (final AnnotatedMethod<?> method : annotationMethods)
      {
         Bean<?> bean = new BeanBuilder<AnnotationAdapterUICommand>(manager)
                  ...
                  .beanLifecycle(new ContextualLifecycle<AnnotationAdapterUICommand>()
                  {
                     ...
                     {
                        InputComponentProducer factory = BeanManagerUtils.getContextualInstance(manager,
                                 InputComponentProducer.class);
                        ...
                        for (AnnotatedParameter<?> parameter : method.getParameters())
                        {
                           ...
                           {
                              input = factory.createSelectOne(paramName, paramType);
                              factory.preconfigureInput(input, withAttributes);
                           }
                           else
                           {
                              input = factory.createInput(paramName, paramType);
                              factory.preconfigureInput(input, withAttributes);
                           }
                           inputs.add(input);
                        }
